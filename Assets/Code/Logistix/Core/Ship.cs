...
namespace Logistix.Core
{
    public class Ship : MonoBehaviour
    {
        ...
        {
            Tower.sprite = Towers[Math.Min(Towers.Length - 1, ShipUpgrade.GetUpgrade(Upgrade.Tower))];
            Side.sprite = Sides[Math.Min(Sides.Length - 1, ShipUpgrade.GetUpgrade(Upgrade.Life))];
        }
